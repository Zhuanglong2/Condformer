...
class TransConvFPN(nn.Module):
    ...
    def __init__(self,
        ...
        self.fusion = nn.Conv2d(64 * 2, 64, 1)
        self.downconv = nn.Conv2d(512, 64, 1)
    ...
    def forward(self, src):
        ...
        if self.attention:
            ...
            trans_feat =self.downconv(src[self.trans_idx])
        inputs = src[:-1]
        ...
        for i in range(used_backbone_levels - 1, 0, -1):
            ...
            # if i == 1:
            #     laterals[i - 1] = self.def_convs(laterals[i - 1])
        # build outputs
        ...
        size = outs[0].shape[2:]
        features = outs[0] + F.interpolate(outs[1], size, mode='bilinear', align_corners=False)
        outs[0] = self.fusion(torch.cat((outs[0], features), dim=1))
