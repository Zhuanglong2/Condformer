...
def draw_umich_gaussian(heatmap, center, radius,num, k=1):
    k = 1
    # radius = 6
    valid_points = []
    # if len(center) > 5:
    #     index = random.sample(range(0, len(center)), 5)
    #     for i in index:
    #         Center = center[i]
    #         diameter = 2 * radius + 1
...
def draw_umich_gaussian2(heatmap, center, radius, k=1):
    diameter = 2 * radius + 1
    gaussian = gaussian2D((diameter, diameter), sigma=diameter / 6)
    x, y = int(center[0]), int(center[1])
    height, width = heatmap.shape[0:2]
...
#             valid_points.append([CT[0], CT[1]])
#
#     return [valid_points, valid_points]
...
def select_mask_points(ct, r, shape, max_sample=5):
    ...
    valid_points = []
    # valid_onepoints = []
    # valid_onepoints.append([CT[0], CT[1]])
    # r = max(int(r // 2), 1)
...
def select_mask_points2(ct, r, shape, max_sample=5):
...
class CollectLane(Collect):
    ...
    def target(self, results):
        def min_dis_one_point(points, idx):
            ...
            for i in range(len(points)):
                if i == idx:
                    ...
                    d = cal_dis(points[idx][0], points[i][0])
                    if d < min_dis:
                        min_dis = d
            return min_dis
        ...
        gt_hm2 = np.zeros((1, hm_h*2, hm_w*2), np.float32)
        gt_masks = []
        gt_masks2 = []
        ...
        for pts in gt_points:
            ...
            pts2 = convert_list(pts, 1)
            pts2 = sorted(pts2, key=cmp_to_key(lambda a, b: b[-1] - a[-1]))
            pts2 = clamp_line(pts2, box=[0, 0, output_w - 1,  output_h - 1], min_length=1)
            if pts2 is not None and len(pts2) > 1:
                valid_gt2.append([pts2, id_class - 1])
        ...
        for l in valid_gt:
            ...
            for i in range(len(l[0])):
                point.append((l[0][i][0] * ratio_hm_mask, l[0][i][1] * ratio_hm_mask))
        ...
        for l in valid_gt2:
            ...
            point2 = []
            for i in range(len(l[0])):
                point2.append((l[0][i][0] * ratio_hm_mask, l[0][i][1] * ratio_hm_mask))
            gt_hm_lane_ends2.append([point2, l[0]])
        ...
        for i, p in enumerate(gt_hm_lane_ends2):
            r = self.radius
            radius2.append(r)
        ...
        if len(gt_hm_lane_ends2) >= 2:
            endpoints2 = [p[0] for p in gt_hm_lane_ends2]
            for j in range(len(endpoints2)):
                dis2 = min_dis_one_point(endpoints2, j)
                if dis2 < 1.5 * radius2[j]:
                    radius2[j] = int(max(dis2 / 1.5, 1) + 0.49999)

        for (end_point, line), r, (end_point2, line2), r2 in zip(gt_hm_lane_ends, radius, gt_hm_lane_ends2, radius2):
            ...
            for i in range(len(end_point)):
                pt_int.append((int(end_point[i][0]), int(end_point[i][1])))
            for i in range(len(end_point2)):
                pt_int2.append((int(end_point2[i][0]), int(end_point2[i][1])))

            #挑选中间点
            # index =int(len(pt_int)/2-1)
            CT.append(pt_int[0])
            # CT.append(pt_int[index])

            draw_umich_gaussian(gt_hm[0], CT, r, num=0)  # 画 start hm图
            # draw_umich_gaussian2(gt_hm2[0], pt_int2, r2, num=0)  # 画 start hm图
            # draw_umich_gaussian(gt_hm[0], CT, r, num=0)#画 start hm图
            # draw_umich_gaussian(gt_hm2[0], CT, r, num=1)#画 mid hm图
            line_array = np.array(line)
            y_min, y_max = int(np.min(line_array[:, 1])), int(np.max(line_array[:, 1]))
            mask_points = select_mask_points(CT, r, (hm_h, hm_w), max_sample=self.max_mask_sample)#选取送入train的点

            pos2 = np.zeros((1, mask_h, mask_w), np.float32)
            pos_mask2 = np.zeros((1, mask_h, mask_w), np.float32)
            reg = np.zeros((1, mask_h, mask_w), np.float32) #计算每个点和gt的offst !!!(这里放的只是一幅图中的一条车道线)!!!
            reg_mask = np.zeros((1, mask_h, mask_w), np.float32) #车道线的mask    !!!(这里放的只是一幅图中的一条车道线)!!!
            ...
            polygon_map = draw_label(m, polygon, 1, 'line', width=self.line_width + 9) > 0

            for y in range(polygon_map.shape[0]):
                for x in np.where(polygon_map[y, :])[0]:
                    reg_x, _ = get_line_intersection(x, y, line)
                    # reg_x2, _ = get_line_intersection(x*8, y*8, line2)
                    # kps and kps_mask:
                    if reg_x is not None:
                        offset = reg_x - x #计算offset
                        reg[0, y, x] = offset
                        if abs(offset) < 10:
                            reg_mask[0, y, x] = 1
        ...
        results['gt_hm'] = DC(to_tensor(gt_hm).float(), stack=True, pad_dims=None)
        results['gt_hm2'] = DC(to_tensor(gt_hm2).float(), stack=True, pad_dims=None)
...
class CollectRNNLanes(Collect):
    ...
    def target(self, results):
        ...
        for (end_point, lines, _), r in zip(gt_hm_lane_ends, radius):
            ...
            draw_umich_gaussian2(gt_hm[0], pt_int, r)
            mask_points = select_mask_points2(
